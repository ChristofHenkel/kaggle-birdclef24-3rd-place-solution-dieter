...
#         print("Group", cfg.group)
    ...
    for epoch in range(cfg.epochs):
        ...
        if cfg.test & (cfg.test_epochs > 0):
            if (epoch + 1) % cfg.test_epochs == 0:
                if not (epoch + 1) == cfg.epochs:
                    test_score = run_eval(model, test_dataloader, cfg, pre="test", curr_epoch=epoch)
